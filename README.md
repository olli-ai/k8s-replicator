...
This is especially useful because the generated secret or configMap is not managed by helm and won't be erased by helm deployments, thus avoiding the secret or configMap to be shortly reset at each deployment, which may trigger the pods to restart if a [reloader](https://github.com/stakater/Reloader) is used.
